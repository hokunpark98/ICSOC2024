...
def run_proposed_method(namespace, prometheus_url):
    ...
    service_routes_proposed, service_versions_proposed = traffic_allocation(v1, prom, namespace, dag, "proposed")
    #service_routes_localization, service_versions_localization = traffic_allocation(v1, prom, namespace, dag, "localization")

    service_routes = sorted(service_routes, key=lambda x: x[0])
    
    print('service_routes_proposed (sorted)')
    for service_route in service_routes_proposed:
        print(service_route)

    # 정보를 반영함  
    apply_destination_rules(namespace, custom_objects_api, service_versions_proposed)  

    proposed_path = "/home/dnc/master/paper2024/trafficyaml"
    apply_virtual_service(custom_objects_api, service_routes_proposed, namespace, proposed_path)
