...
def localization(upstream_nodes, downstream_nodes, edge_weights):
    flow_dict = defaultdict(dict)
    for u_node in upstream_nodes:
        closest_d_node = min(downstream_nodes, key=lambda d: edge_weights[(u_node, d)])
        in_node = f"{closest_d_node}_in"
        out_node = f"{u_node}_out"
        flow_dict[out_node][in_node] = 100
    return flow_dict

def traffic_allocation(v1, prom, namespace, dag, method):
    ...
    for pair in service_pairs:
        ...
        elif method == 'localization':
            flow_dict = localization(upstream_nodes, downstream_nodes, edge_weights)
