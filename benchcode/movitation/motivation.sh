...
# QPS 값 목록
qps_values=(10 50 100 150 200 250)
paper_arg1="paper2"
...
for ((i=1; i<=repetitions; i++)); do
    ...
    for QPS in "${qps_values[@]}"; do
        ...
        echo "optTraffic..."
        kubectl create -Rf /home/dnc/master/paper2024/trafficyaml/motivation
        sleep 10

        # 두 번째 wrk2 명령어 실행 및 결과 추가 저장
        echo "Running second wrk2 load test with QPS=$QPS..."
        wrk2 -t4 -c10 -d180s -R$QPS --latency "http://10.105.92.109:11003/d?value=1"  | tee -a "$result_dir/result${QPS}_2.txt"
    done
done
