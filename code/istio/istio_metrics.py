...
def get_request_durations(prom, namespace):
    query = (
        f'('
        f'istio_request_duration_milliseconds_sum{{kubernetes_namespace="{namespace}"}} - '
        f'istio_request_duration_milliseconds_sum{{kubernetes_namespace="{namespace}"}} offset 280s) '
        f'/'
        f'(istio_requests_total{{kubernetes_namespace="{namespace}"}} - '
        f'istio_requests_total{{kubernetes_namespace="{namespace}"}} offset 280s)'
    )
    result = prom.custom_query(query)
    return parse_istio_metrics(result)
...
def get_request_durations(prom, namespace):
    query = (
        f'rate(istio_request_duration_milliseconds_sum{{kubernetes_namespace="{namespace}"}}[280s]) '
        f'/ '
        f'rate(istio_requests_total{{kubernetes_namespace="{namespace}"}}[280s])'
    )
